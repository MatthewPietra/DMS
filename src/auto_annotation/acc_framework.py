...
class ACCFramework:
    ...
    def calculate_scores(
        ...
        details = {
            ...
            "consistency_details": self._get_consistency_details(
    ...
    def _calculate_consistency(
        ...
        for hist_entry in recent_annotations[:-1]:  # Exclude current entry
            ...
            consistency_score = self._calculate_annotation_similarity(
    ...
    def _store_annotation_history(
        ...
        if len(self.annotation_history[image_path]) > 10:
            self.annotation_history[image_path] = self.annotation_history[image_path][
                -10:
            ]
    ...
    def _calculate_annotation_similarity(
        ...
        count_similarity = 1.0 - abs(len(annotations1) - len(annotations2)) / max(
    ...
    def _get_accuracy_details(
        ...
        if not ground_truth:
            ...
            return {
                ...
                "precision": (
                    len(annotations) / len(ground_truth) if ground_truth else 0.0
                ),
            }
    ...
    def _get_credibility_details(
        ...
        return {
            ...
            "high_confidence_ratio": (
                len([c for c in confidences if c > 0.8]) / len(confidences)
                if confidences
                else 0.0
            ),
        }
    ...
    def calculate_ensemble_scores(
        ...
        details = {
            ...
            "agreement_metrics": self._calculate_ensemble_agreement_metrics(
                model_results
            ),
        }
    ...
    def get_improvement_recommendations(self, acc_scores: ACCScores) -> List[str]:
        ...
        if acc_scores.consistency < self.consistency_threshold:
            recommendations.append(
                "Ensure consistent preprocessing and input conditions"
            )
            recommendations.append("Consider model fine-tuning for specific domain")

        if not recommendations:
            recommendations.append(
                "Quality is good - consider gradual threshold increases"
            )
