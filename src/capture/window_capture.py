...
class WindowDetector:
    ...
    def get_available_windows(self, include_minimized: bool = False) -> List[WindowInfo]:
    ...
    def _get_windows_pygetwindow(self, include_minimized: bool) -> List[WindowInfo]:
        ...
        try:
            for window in gw.getAllWindows():
                ...
                is_minimized = window.isMinimized if hasattr(window, 'isMinimized') else False
...
class CaptureSession:
    ...
    def start_capture(self):
        ...
        self.capture_thread = threading.Thread(
            ...
            name=f"capture_{self.session_id}",
            daemon=True
        )
        ...
        while self.is_active and not self.stop_event.is_set():
            try:
                ...
                if self.stats.total_duration > 0:
                    self.stats.average_fps = self.stats.images_captured / self.stats.total_duration
    ...
    def _capture_frame(self) -> bool:
        ...
        try:
            ...
            filename = f"{self.session_id}_{timestamp}.{self.config.image_format.lower()}"
...
class WindowCaptureSystem:
    ...
    def find_window(self, title: str, partial_match: bool = True) -> Optional[WindowInfo]:
        """Find window by title."""
        return self.window_detector.find_window_by_title(title, partial_match)
    
    def start_session(self, session_id: str, output_dir: Path, 
                     window_title: str = None, fps: int = None, 
                     resolution: Tuple[int, int] = None) -> CaptureSession:
        ...
        if window_title:
            ...
            if window:
                ...
                self.logger.warning(f"Window '{window_title}' not found, using full screen")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="YOLO Vision Studio - Window Capture System")
    ...
    parser.add_argument("--duration", type=int, default=10, help="Demo duration in seconds (default: 10)")
    parser.add_argument("--output-dir", type=str, default="data/temp", help="Output directory for captures")
            ...
            try:
                ...
                while (time.time() - start_time) < args.duration:
                    ...
                    if (time.time() - last_status_time) >= 2:
                        ...
                        print(f"   📊 Captured: {stats['images_captured']} images, "
