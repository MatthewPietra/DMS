...
class ConfigManager:
    ...
    def __init__(self, config_path: Optional[Union[str, Path]] = None):
        ...
        if self.main_config_path.exists():
            ...
            self.logger.info(f"Config file not found at {self.main_config_path}, using defaults")
        ...
        try:
            ...
            for config_key, config_class in [
                ...
                try:
                    ...
                    filtered_config = {k: v for k, v in config_data.items() if k in expected_fields}
    ...
    def set(self, key: str, value: Any):
        ...
        if keys[0] in self._config_objects:
            try:
                ...
                self.logger.warning(f"Failed to update config object for {keys[0]}: {e}")
    ...
    def validate_config(self) -> List[str]:
        ...
        if not capture_config.min_fps <= capture_config.default_fps <= capture_config.max_fps:
            issues.append("capture fps values must satisfy: min_fps <= default_fps <= max_fps")
        ...
        if not 0.0 <= auto_config.auto_accept_threshold <= 1.0:
            issues.append("auto_annotation.auto_accept_threshold must be between 0.0 and 1.0")
    ...
    def apply_environment_overrides(self):
        ...
        for key, value in os.environ.items():
            if key.startswith(env_prefix):
                ...
                self.logger.info(f"Applied environment override: {config_key} = {parsed_value}")
