...
class QualityMetrics:
    ...
    def calculate_iou_matrix(self, pred_boxes: List[BoundingBox], 
                           gt_boxes: List[BoundingBox]) -> np.ndarray:
    ...
    def calculate_precision_recall(self, pred_boxes: List[BoundingBox],
    ...
    def calculate_ap(self, precisions: List[float], recalls: List[float]) -> float:
        ...
        for t in np.arange(0, 1.1, 0.1):
            ...
            valid_precisions = [p for r, p in zip(recalls_sorted, precisions_sorted) if r >= t]
            max_precision = max(valid_precisions) if valid_precisions else 0.0
            ap += max_precision
        
        return ap / 11.0
    
    def calculate_map(self, annotations_dict: Dict[str, Tuple[List[BoundingBox], List[BoundingBox]]],
    ...
    def calculate_annotation_quality_score(self, annotation_set: AnnotationSet) -> Dict[str, float]:
        ...
        if len(class_counts) > 0:
            ...
            distribution_score = 1.0 - (np.std(count_values) / (np.mean(count_values) + 1e-6))
            scores["class_distribution"] = max(0.0, min(1.0, distribution_score))
        
        # Overall quality score
        scores["quality_score"] = (
            scores["box_quality"] * 0.6 +
            scores["class_distribution"] * 0.4
        )
...
class ACCFramework:
    ...
    def calculate_accuracy(self, pred_annotations: List[AnnotationSet],
        ...
        map_results = self.quality_metrics.calculate_map(annotations_dict, iou_threshold)
    ...
    def calculate_credibility(self, annotations: List[AnnotationSet],
                            ensemble_predictions: Optional[List[List[AnnotationSet]]] = None) -> Dict[str, float]:
        ...
        for annotation in annotations:
            ...
            if ensemble_predictions:
                ...
                image_ensembles = [
                    ensemble for ensemble in ensemble_predictions
                    if any(ann.image_id == annotation.image_id for ann in ensemble)
                ]
                
                if image_ensembles:
                    agreement_score = self._calculate_ensemble_agreement(image_ensembles[0])
        ...
        return {
            ...
            "ensemble_agreement": np.mean(agreement_scores) if agreement_scores else None
        }
    
    def calculate_consistency(self, annotations: List[AnnotationSet],
    ...
    def calculate_acc_score(self, pred_annotations: List[AnnotationSet],
                           gt_annotations: Optional[List[AnnotationSet]] = None,
                           ensemble_predictions: Optional[List[List[AnnotationSet]]] = None) -> Dict[str, Any]:
        ...
        credibility_results = self.calculate_credibility(pred_annotations, ensemble_predictions)
    ...
    def _calculate_annotation_similarity(self, ann1: AnnotationSet, ann2: AnnotationSet) -> float:
    ...
    def get_quality_trend(self, window_size: int = 10) -> Dict[str, List[float]]:
        ...
        for entry in recent_history:
            ...
            trends["credibility_scores"].append(results["credibility"]["credibility_score"])
            trends["consistency_scores"].append(results["consistency"]["consistency_score"])
...
class MetricsCalculator:
    ...
    def calculate_precision_recall(self, pred_boxes: List[BoundingBox],
        ...
        return self.quality_metrics.calculate_precision_recall(pred_boxes, gt_boxes, iou_threshold)
    ...
    def calculate_acc_score(self, pred_annotations: List[AnnotationSet],
                           ...
                           ensemble_predictions: Optional[List[List[AnnotationSet]]] = None) -> Dict[str, Any]:
        """Calculate ACC score."""
        return self.acc_framework.calculate_acc_score(pred_annotations, gt_annotations, ensemble_predictions)
    
    def calculate_accuracy(self, pred_annotations: List[AnnotationSet],
                         gt_annotations: List[AnnotationSet],
                         iou_threshold: float = 0.5) -> Dict[str, float]:
        """Calculate accuracy metrics."""
        return self.acc_framework.calculate_accuracy(pred_annotations, gt_annotations, iou_threshold)
    
    def calculate_credibility(self, annotations: List[AnnotationSet],
                            ensemble_predictions: Optional[List[List[AnnotationSet]]] = None) -> Dict[str, float]:
        """Calculate credibility metrics."""
        return self.acc_framework.calculate_credibility(annotations, ensemble_predictions)
    
    def calculate_consistency(self, annotations: List[AnnotationSet],
                            similarity_threshold: float = 0.8) -> Dict[str, float]:
        """Calculate consistency metrics."""
        return self.acc_framework.calculate_consistency(annotations, similarity_threshold)
...
def calculate_cohens_kappa(annotator1: List[AnnotationSet], 
