...
class HardwareDetector:
    ...
    def _detect_system(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"System detected: {self._system_info.os_name} {self._system_info.os_version}")
            self.logger.info(f"CPU: {self._system_info.cpu_name} ({self._system_info.cpu_count} cores)")
            self.logger.info(f"Memory: {self._system_info.memory_available}GB available / {self._system_info.memory_total}GB total")
            ...
            self._system_info = SystemInfo(
                ...
                cpu_count=1, cpu_name="Unknown",
                memory_total=8, memory_available=4,
                python_version=sys.version.split()[0],
                pytorch_version=torch.__version__ if TORCH_AVAILABLE else "Not installed"
            )
    ...
    def _detect_cuda_gpus(self) -> List[GPUInfo]:
        ...
        try:
            ...
            for i in range(cuda_device_count):
                ...
                memory_free = memory_total - (torch.cuda.memory_allocated(i) // (1024**2))
    ...
    def _select_optimal_device(self) -> None:
        ...
        cuda_gpus = [gpu for gpu in self._gpu_info if gpu.device_type == DeviceType.CUDA]
        if cuda_gpus:
            ...
            self.logger.info(f"Selected device: {self._selected_device} ({best_gpu.name})")
            return
        
        # Use DirectML if available
        directml_gpus = [gpu for gpu in self._gpu_info if gpu.device_type == DeviceType.DIRECTML]
        if directml_gpus:
    ...
    def get_optimal_batch_size(self, image_size: int = 640, model_size: str = "n") -> int:
    ...
    def configure_torch_settings(self) -> None:
        ...
        if not TORCH_AVAILABLE:
            self.logger.warning("PyTorch not available, skipping optimization configuration")
            return
        ...
        try:
            if self._device_type == DeviceType.CUDA:
                ...
                self.logger.info(f"CPU optimizations applied ({self._system_info.cpu_count} threads)")
