...
class AnnotationInterface(QMainWindow if PYQT_AVAILABLE else object):
    ...
    def _create_left_panel(self) -> QWidget:
        ...
        for tool_id, tool_name, shortcut in tools:
            ...
            btn.toggled.connect(lambda checked, t=tool_id: self._set_tool(t) if checked else None)
    ...
    def _setup_shortcuts(self):
        ...
        QShortcut(QKeySequence(shortcuts.get('next_image', 'Right')), self, self._next_image)
        QShortcut(QKeySequence(shortcuts.get('prev_image', 'Left')), self, self._previous_image)
        ...
        # Zoom shortcuts
        QShortcut(QKeySequence(shortcuts.get('zoom_in', 'Ctrl+Plus')), self, self._zoom_in)
        QShortcut(QKeySequence(shortcuts.get('zoom_out', 'Ctrl+Minus')), self, self._zoom_out)
        QShortcut(QKeySequence(shortcuts.get('fit_to_window', 'Ctrl+0')), self, self._fit_to_window)
    ...
    def _save_annotations(self):
        ...
        try:
            data = {
                ...
                'image_size': [self.image_label.pixmap().width(), self.image_label.pixmap().height()],
        ...
        if current_item:
            ...
            if 0 <= ann_index < len(self.current_annotations):
                ...
                duplicate = Annotation(
                    ...
                    coordinates=[c + 10 for c in original.coordinates],  # Offset by 10 pixels
...
class AnnotationCanvas(QLabel):
    ...
    def _draw_annotation(self, painter: QPainter, ann: Annotation, highlighted: bool = False):
        ...
        if ann.annotation_type == 'bbox':
            ...
            if len(coords) >= 6:  # At least 3 points
                points = [QPoint(int(coords[i]), int(coords[i+1])) 
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        if self.parent_interface.current_tool == 'polygon' and len(self.current_points) >= 3:
...
class ClassDialog(QDialog if PYQT_AVAILABLE else object):
    ...
    def _setup_ui(self):
        ...
        self.color_button.setStyleSheet(f"background-color: {self.class_info.get('color', '#FF0000')}")
    ...
    def _choose_color(self):
        """Open color picker."""
        color = QColorDialog.getColor(QColor(self.class_info.get('color', '#FF0000')), self)
        if color.isValid():
...
def launch_annotation_interface(config_manager: ConfigManager) -> Optional[AnnotationInterface]:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="YOLO Vision Studio - Annotation Interface")
    parser.add_argument("--config", type=str, help="Path to config file")
    parser.add_argument("--images-dir", type=str, help="Directory containing images to annotate")
