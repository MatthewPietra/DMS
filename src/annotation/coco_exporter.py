...
class COCOExporter:
    ...
    def export_dataset(self, 
        ...
        try:
            ...
            if export_format.upper() == "COCO":
                return self._export_coco_format(project_path, output_path, classes, include_images)
            elif export_format.upper() == "YOLO":
                return self._export_yolo_format(project_path, output_path, classes, include_images)
            elif export_format.upper() == "PASCAL_VOC":
                return self._export_pascal_voc_format(project_path, output_path, classes, include_images)
            else:
    ...
    def _export_coco_format(self, 
        ...
        for image_file in images_dir.glob("*.jpg"):
            ...
            if annotation_file.exists():
                try:
                    ...
                    self.logger.warning(f"Failed to process annotation {annotation_file}: {e}")
        ...
        self.logger.info(f"COCO export completed: {len(coco_data['images'])} images, "
                        f"{len(coco_data['annotations'])} annotations")
        
        return True
        
    def _convert_to_coco_annotation(self, 
    ...
    def _convert_to_yolo_annotation(self, 
                                  annotation: Dict[str, Any],
                                  image_width: int,
                                  image_height: int) -> Optional[str]:
    ...
    def _export_pascal_voc_format(self, 
                                 project_path: Path,
                                 output_path: Path,
                                 classes: List[str],
                                 include_images: bool) -> bool:
        ...
        for image_file in images_dir.glob("*"):
            ...
            if annotation_file.exists():
                try:
                    ...
                    for ann in annotation_data.get("annotations", []):
                        obj_element = self._convert_to_pascal_voc_object(ann, classes, width, height)
        ...
        self.logger.info(f"Pascal VOC export completed: {processed_count} images processed")
        return True
        
    def _convert_to_pascal_voc_object(self, 
                                    annotation: Dict[str, Any],
                                    classes: List[str],
                                    image_width: int,
                                    image_height: int) -> Optional["Element"]:
