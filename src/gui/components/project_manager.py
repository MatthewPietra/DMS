...
from PySide6.QtWidgets import (
    QLabel, QVBoxLayout, QWidget, QPushButton, QListWidget, QHBoxLayout, QInputDialog, QLineEdit, QMessageBox, QFileDialog, QDialog, QFormLayout, QDialogButtonBox
)
...
class ProjectManagerWidget(QWidget):
    ...
    def refresh_project_list(self):
        ...
        if not projects_found:
            self.project_list.addItem("No projects found. Create a new project to get started.")
    ...
    def new_project(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if not classes:
                QMessageBox.warning(self, "Validation Error", "At least one class must be specified.")
                return
    ...
    def open_project(self):
        ...
        if not item:
            QMessageBox.information(self, "Select Project", "Please select a project to open.")
            return
        ...
        if not project_path.exists():
            ...
            return
        # Set as current project in main window
        self.main_window.set_current_project(str(project_path))
        QMessageBox.information(self, "Project Opened", f"Opened project: {project_name}")
    ...
    def get_project_info(self, project_name: str) -> dict:
        ...
        if not config_file.exists():
            return {"name": project_name, "description": "", "classes": [], "error": "Config not found"}
        ...
        try:
            ...
            return {"name": project_name, "description": "", "classes": [], "error": f"Config error: {e}"}

    def project_settings(self):
        item = self.project_list.currentItem()
        if not item:
            QMessageBox.information(self, "Select Project", "Please select a project to edit settings.")
            return
    ...
    def import_project(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Import Error", f"Failed to read config file: {e}")
            return
            
        name = config.get("name")
        if not name:
            QMessageBox.warning(self, "Import Error", "Invalid project config: missing project name.")
            return
            
        project_path = self.projects_dir / name
        if project_path.exists():
            QMessageBox.warning(self, "Import Error", f"Project '{name}' already exists.")
            return
        ...
        try:
            ...
            with open(project_path / "config.json", "w", encoding="utf-8") as f:
                json.dump(config, f, indent=2)
            self.refresh_project_list()
            QMessageBox.information(self, "Import Success", f"Project '{name}' imported successfully.")
        except Exception as e:
    ...
    def export_project(self):
        item = self.project_list.currentItem()
        if not item:
            QMessageBox.information(self, "Select Project", "Please select a project to export.")
            return
        ...
        project_path = self.projects_dir / project_name
        if not project_path.exists():
            QMessageBox.warning(self, "Export Error", f"Project '{project_name}' not found.")
            return
...
class ProjectDialog(QDialog):
    def __init__(self, parent=None, title="", name="", description="", classes=None):
        ...
        self.classes_edit.setPlaceholderText("Enter classes separated by commas (e.g., person, car, bike)")
        ...
        # Add help text
        help_label = QLabel("Classes are the object types you want to detect in your project.")
    ...
    def validate_and_accept(self):
        ...
        if not classes:
            QMessageBox.warning(self, "Validation Error", "At least one valid class must be specified.")
            self.classes_edit.setFocus()
            return
