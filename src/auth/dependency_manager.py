...
class AuthenticationDependencyManager:
    ...
    def ensure_authentication_ready(self) -> Tuple[bool, List[str]]:
        ...
        if not self.check_dependency("requests"):
            print("⚠️  'requests' module not found. Installing authentication dependencies...")
            return self.install_missing_dependencies()
        
        # Check other critical dependencies
        critical_deps = ["pycryptodome", "psutil"]
        missing_critical = [dep for dep in critical_deps if not self.check_dependency(dep)]
