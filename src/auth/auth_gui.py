...
class AuthenticationGUI(QMainWindow):
    ...
    # Signals
    authentication_successful = pyqtSignal(dict) if QT_VERSION == "PyQt5" else Signal(dict)
...
class KeyAuthVerificationThread(QThread):
    ...
    verification_complete = pyqtSignal(bool, dict) if QT_VERSION == "PyQt5" else Signal(bool, dict)
    ...
    def run(self):
        ...
        try:
            ...
            if success:
                ...
                self.verification_complete.emit(False, {'error': 'License verification failed'})
