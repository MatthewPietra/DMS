...
try:
    ...
    try:
        ...
        from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QCheckBox, QMessageBox
        GUI_AVAILABLE = True
    except ImportError:
        try:
            from PyQt6.QtWidgets import QApplication, QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QCheckBox, QMessageBox
            from PyQt6.QtCore import Qt
            GUI_AVAILABLE = True
        except ImportError:
            try:
                from PySide6.QtWidgets import QApplication, QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QCheckBox, QMessageBox
...
class LauncherPreferences:
    ...
    def should_show_mode_selection(self) -> bool:
        ...
        return (
            self.preferences.get("show_mode_selection", True) and
            not self.preferences.get("user_choices", {}).get("skip_mode_dialog", False)
        )
    ...
    def _gui_authentication(self) -> bool:
        ...
        try:
            ...
            if result and result.get('user_data'):
                ...
                print(f"✅ Authentication successful! Welcome, {self.authenticated_user['username']}")
                return True
