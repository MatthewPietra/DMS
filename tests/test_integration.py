...
from yolo_vision_studio.studio import YOLOVisionStudio
from yolo_vision_studio.config import YOLOVisionConfig
from yolo_vision_studio.utils.hardware import HardwareDetector
from yolo_vision_studio.utils.metrics import BoundingBox
...
class TestProjectWorkflow(unittest.TestCase):
    ...
    def test_project_creation_workflow(self):
        ...
        expected_dirs = [
            'images', 'annotations', 'models', 'exports', 
            'logs', 'config', 'cache'
        ]
    ...
    def test_training_workflow(self, mock_train):
        ...
        # Test training
        result = self.studio.train_model(
            model_name='yolov8n',
            epochs=1,
            batch_size=2
        )
...
class TestAutoAnnotationWorkflow(unittest.TestCase):
    ...
    @patch('yolo_vision_studio.auto_annotation.auto_annotator.AutoAnnotator.annotate_batch')
...
class TestHardwareIntegration(unittest.TestCase):
    ...
    def test_configuration_hardware_integration(self):
        ...
        specs = self.detector.detect_hardware()
        if specs.gpus:
            optimal_device = 'cuda' if any(gpu.device_type == 'CUDA' for gpu in specs.gpus) else 'directml'
        else:
            optimal_device = 'cpu'
